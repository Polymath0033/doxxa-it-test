...
export const Modal: React.FC<{
  ...
  const ModalOverlay = () => (
    <dialog
      open
      className={`fixed z-50  top-1/2 left-1/2 -translate-x-1/2  w-[calc(100%_-_70px)] sm:w-[calc(100%_-_100px)]  md:w-[40rem] max-h-[calc(100vh_-_50px)]  -translate-y-1/2 rounded-2xl overflow-y-auto overflow-x-hidden h-fit py-8  px-5 ${className}`}
    >
      <button
        onClick={closeHandler}
        type="button"
  ...
  return (
    <React.Fragment>
      ...
      {createPortal(<ModalOverlay />, document.getElementById("modal-root")!)}
    </React.Fragment>
  );
};
