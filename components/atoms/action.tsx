...
export const MoreActions: FC<{
  ...
  isOpen: boolean;
  name: string;
}> = ({ id, closeHandler, isOpen, name }) => {
  const [updateModal, setUpdateModal] = useState(false);
  const [deleteModal, setDeleteModal] = useState(false);
  const updateModalHandler = () => {
    console.log("isOpen");
    console.log(isOpen);
    console.log("Modal", updateModal);
  ...
  return (
    <Fragment>
      ...
      <ul className="bg-white absolute top-full right-0 rounded-xl justify-start *:text-start w-[135px] shadow-[0px_0px_25.061px_0px_rgba(161,_161,_161,_0.25)] flex flex-col gap-2 *:text-[15px] *:font-semibold text-08  px-3 py-4 z-50">
        ...
        <li>
          ...
          {updateModal && (
            <UpdateInventory
              modal={updateModal}
              id={id}
              closeHandler={() => setUpdateModal(!updateModal)}
            />
          )}
        </li>
        <li>
          <div className="cursor" onClick={deleteModalHandler}>
            Delete
          </div>
        </li>
      </ul>
      ...
      {deleteModal && (
        <DeleteInventory
          ...
          name={name}
          modal={deleteModal}
          closeHandler={deleteModalHandler}
